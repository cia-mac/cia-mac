...
if /bin/bash -n housekeeper.sh; then echo "  OK"; else echo "  FAILED"; fail=1; fi

echo "== 3. safety test suite =="
ENG="$PWD/housekeeper.sh" INSTALLER="$PWD/install-auto-cleanup.command" /bin/bash tests-v9.sh || fail=1

echo "== 4. rendered plist lint =="
if command -v plutil >/dev/null 2>&1; then
  tmp=$(mktemp "${TMPDIR:-/tmp}/hk_plist.XXXXXX") || { echo "  mktemp failed"; fail=1; }
  if [ -n "${tmp:-}" ]; then
    awk -v label="com.ciamac.housekeeper" \
        -v engine="$HOME/.cleanup/housekeeper.sh" \
        -v home="$HOME" '
      { gsub(/\$LABEL/, label); gsub(/\$ENGINE/, engine); gsub(/\$HOME/, home); print }
    ' com.ciamac.housekeeper.plist.template > "$tmp"
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "You may now run the installer:  open install-auto-cleanup.command"
else
